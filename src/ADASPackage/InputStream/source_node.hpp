...
{
	Source_Parameters():  fps(10), paused(false),stopped(false) {}
	double fps;
...
// Create a subscription on the input topic which prints on receipt of new messages.
 ctrlsub_ = this->create_subscription<adas_interfaces::msg::SourceParameters>("ADAS_command", [this] (adas_interfaces::msg::SourceParameters::UniquePtr msg)
  {

		  if(msg)
			  this->source_.paused.store(msg->paused);
		      this->source_.SourcePath = msg->dir;

		      std::cout<<"==================  "   <<    msg->inputmode <<"   =========================="<<endl;

		      if(this->source_.paused.load()==true)
		      {
		    	  std::cout<<"======================================================"<<endl;
		    	  std::cout<< "              Pausing the Source                     "<<endl;
		    	  std::cout<<"======================================================"<<endl;
...
public:
 ...
 Source_ADAS(Source inputmode, std::string path, const std::string output_topic, const std::string & node_name = "source_node") : Node(node_name, true), path_("cropped/cropped_%d.png"), inputmode_(Source::FOLDER)

{

	 /*

	 auto parameter_service = std::make_shared<rclcpp::parameter_service::ParameterService>(shared_from_this());
	 auto parameters_client = std::make_shared<rclcpp::parameter_client::AsyncParametersClient>(shared_from_this());
	 ...
	 // Setup callback for changes to parameters.
	   ...
	   auto set_parameters_results = parameters_client->set_parameters({
	     ...
	     rclcpp::spin_until_future_complete(shared_from_this(), set_parameters_results);
	...
	*/
    ...
    // Create a subscription on the input topic which prints on receipt of new messages.
      ctrlsub_ = this->create_subscription<adas_interfaces::msg::SourceParameters>("ADAS_command_pipeline", [this] (adas_interfaces::msg::SourceParameters::UniquePtr msg)
       {
