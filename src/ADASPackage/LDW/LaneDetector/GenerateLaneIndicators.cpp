...
void GetEWVAR(const int& sampleIdx,
              int& sigmaWindowSize,
              const double& inputParam,
              const double& outputEWMA,
              double& outputEWVAR)
{
    ...
    if(sampleIdx < NUM_WINDOW_EWM) {
        ...
        outputEWVAR =
            sigmaFactor * outputEWVAR +
            (1 - sigmaFactor) * (inputParam - outputEWMA) * (inputParam - outputEWMA); // outputEWMA: use current EWMA
    }
} // end GetEWVAR
                     ...
                     const double& intervalTime)
{
    ...
    if(sampleIdx > 1) {
        ...
        laneFeatures.LATMEAN_Baseline =
            laneFeatures.LATMEAN_Baseline > laneFeatures.LATMEAN ? laneFeatures.LATMEAN_Baseline : laneFeatures.LATMEAN;
        laneFeatures.LANEDEV_Baseline =
            laneFeatures.LANEDEV_Baseline > laneFeatures.LANEDEV ? laneFeatures.LANEDEV_Baseline : laneFeatures.LANEDEV;
