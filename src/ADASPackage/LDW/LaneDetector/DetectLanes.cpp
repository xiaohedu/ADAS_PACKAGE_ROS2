...
void IPMPreprocess(const cv::Mat& ipmMat, const LaneDetectorConf& laneDetectorConf, cv::Mat& thMat)
{
    ...
    //	cv::Mat kernelSobel  = (cv::Mat_<double>(3,3)<< -1, 0, 1, -2, 0, 2, -1, 0, 1 ); //This kernel has the same
    //effect like Sobel
    //	cv::Mat kernelScharr  = (cv::Mat_<double>(3,3)<< -3, 0, 3,-10, 0, 10,-3, 0, 3 ); //This kernel has the same
    //effect like Scharr
...
void IPMDetectLanes(const cv::Mat& ipmMat,
                    ...
                    std::vector<Lane>& leftIPMLanes,
                    std::vector<Lane>& rightIPMLanes,
                    cv::Mat& leftCoefs,
                    cv::Mat& rightCoefs,
...
/* Curve Fitting Step */
    ...
    if(!rightPointSet.empty()) {
        ...
        FittingCurve_RANSAC(
            rightPointSet, termNum, minDataNum, iterNum, thValue, rightCloseDataNum, rightCoefs, colorMat);
...
/* * This function detects lanes in the input image using IPM transformation and the input camera parameters. The
returned lines
...
void FilterLanes(cv::Mat& laneMat, const LaneDetectorConf& laneDetectorConf)
{
    ...
    case SOBEL_FILTER_1:
        ...
        if(DEBUG_LANE)
            imShowSub("1.Sobel", mat1, WIN_COLS, WIN_ROWS, 1);

        cv::medianBlur(mat1, mat2, 3);
        if(DEBUG_LANE)
            imShowSub("2.Blur", mat2, WIN_COLS, WIN_ROWS, 2);

        cv::threshold(mat2, finalMat, 40, 255, cv::THRESH_BINARY);
        if(DEBUG_LANE)
            imShowSub("3.Threshold", finalMat, WIN_COLS, WIN_ROWS, 3);
    ...
    case SOBEL_FILTER_2:
        ...
        if(DEBUG_LANE)
            imShowSub("1.Sobel_X", mat1, WIN_COLS, WIN_ROWS, 1);

        cv::Sobel(mat1, mat2, CV_8U, 0, 1, 3);
        if(DEBUG_LANE)
            imShowSub("2.Sobel_Y", mat2, WIN_COLS, WIN_ROWS, 2);

        cv::medianBlur(mat2, mat3, 3);
        if(DEBUG_LANE)
            imShowSub("3.Median", mat3, WIN_COLS, WIN_ROWS, 3);

        cv::threshold(mat3, finalMat, 100, 255, cv::THRESH_BINARY);
        if(DEBUG_LANE)
            imShowSub("4.Threshold", finalMat, WIN_COLS, WIN_ROWS, 4);
    ...
    case SOBEL_FILTER_3:
        ...
        if(DEBUG_LANE)
            imShowSub("1.Sobel_Y", mat1, WIN_COLS, WIN_ROWS, 1);

        cv::Sobel(mat1, mat2, CV_8U, 1, 0, 3);
        if(DEBUG_LANE)
            imShowSub("2.Sobel_X", mat2, WIN_COLS, WIN_ROWS, 2);
    ...
    case CANNY_FILTER:
        ...
        if(DEBUG_LANE)
            imShowSub("1.Canny", finalMat, WIN_COLS, WIN_ROWS, 1);
    ...
    case LAPLACE_FILTER:
        ...
        if(DEBUG_LANE)
            imShowSub("1.Laplacian", mat1, WIN_COLS, WIN_ROWS, 1);

        cv::medianBlur(mat1, mat2, 3);
        if(DEBUG_LANE)
            imShowSub("2.Median", mat2, WIN_COLS, WIN_ROWS, 2);
    ...
    case DOG_FILTER:
        ...
        mat2 = laneMat - mat1;
        if(DEBUG_LANE)
            imShowSub("1.DOG", mat2, WIN_COLS, WIN_ROWS, 1);

        cv::GaussianBlur(mat2, mat3, cv::Size(3, 3), 1);
        if(DEBUG_LANE)
            imShowSub("2.Blur", mat3, WIN_COLS, WIN_ROWS, 2);

        cv::threshold(mat2, finalMat, 5, 255, cv::THRESH_BINARY);
        if(DEBUG_LANE)
            imShowSub("3.Threshold", finalMat, WIN_COLS, WIN_ROWS, 4);
    ...
    if(!isROI) {
        ...
        * Matched lines may be the middle of all candidate lines  */
        ...
        for(size_t i = 0; i < leftHfLanesCandi.size(); i++) {
            leftHfLanesCandi[i][1] =
                leftHfLanesCandi[i][0] > 0 ? leftHfLanesCandi[i][1] : leftHfLanesCandi[i][1] - (float)CV_PI;
            leftHfLanesCandi[i][0] = std::abs(leftHfLanesCandi[i][0]);

            if(leftHfLanesCandi[i][0] > laneDetectorConf.rhoMin && leftHfLanesCandi[i][1] < laneDetectorConf.thetaMax &&
        ...
        if(!leftHfLanes.empty()) {
            ...
            for(size_t i = 0; i < leftLanes.size(); i++) {
                ...
                for(size_t j = 0; j < lLanePts.size(); j++) {
                    double dis =
                        std::abs(xIntercept * lLanePts[j].y + yIntercept * lLanePts[j].x - xIntercept * yIntercept) /
        ...
        for(size_t i = 0; i < rightHfLanesCandi.size(); i++) {
            rightHfLanesCandi[i][1] =
                rightHfLanesCandi[i][0] > 0 ? rightHfLanesCandi[i][1] : rightHfLanesCandi[i][1] - (float)CV_PI;
        ...
        if(!rightHfLanes.empty()) {
            ...
            for(int i = 0; i < (int)rightLanes.size(); i++) {
                ...
                for(int j = 0; j < (int)rLanePts.size(); j++) {
                    double dis =
                        std::abs(xIntercept * rLanePts[j].y + yIntercept * rLanePts[j].x - xIntercept * yIntercept) /
                        (xIntercept * xIntercept + yIntercept * yIntercept);
                     ...
                     const int offsetY)
{
    ...
    if(!k.empty()) {
        vp.x = (k.at(0) * lanes.at(0).startPoint.x - k.at(1) * lanes.at(1).startPoint.x - lanes.at(0).startPoint.y +
                lanes.at(1).startPoint.y) /
                   (k.at(0) - k.at(1)) +
               offsetX;

        vp.y = (k.at(0) * lanes.at(1).startPoint.y - k.at(1) * lanes.at(0).startPoint.y +
                k.at(0) * k.at(1) * lanes.at(0).startPoint.x - k.at(0) * k.at(1) * lanes.at(1).startPoint.x) /
                   (k.at(0) - k.at(1)) +
               offsetY;
                ...
                const LaneDetectorConf& laneDetectorConf)
{
    ...
    if(laneKalmanIdx > TH_KALMANFILTER && !pHfLanes.empty()) {
        ...
        if(!isChangeLane) {
            ...
            cv::line(laneMat,
                     cv::Point(lStartX + proStartXRange + offsetX, offsetY),
                     cv::Point(lStartX - proStartXRange + offsetX, offsetY),
                     CV_RGB(100, 0, 100),
                     2);
            cv::line(laneMat,
                     cv::Point(lStartX - proStartXRange + offsetX, offsetY),
                     cv::Point(lEndX - proEndXRange + offsetX, laneMat.rows),
                     CV_RGB(100, 0, 100),
                     2);
            cv::line(laneMat,
                     cv::Point(lEndX - proEndXRange + offsetX, laneMat.rows),
            ...
            cv::line(laneMat,
                     cv::Point(rStartX + proStartXRange + offsetX, offsetY),
                     cv::Point(rStartX - proStartXRange + offsetX, offsetY),
                     CV_RGB(0, 100, 100),
                     2);
            cv::line(laneMat,
                     cv::Point(rStartX - proStartXRange + offsetX, offsetY),
                     cv::Point(rEndX - proEndXRange + offsetX, laneMat.rows),
                     CV_RGB(0, 100, 100),
                     2);
            cv::line(laneMat,
                     cv::Point(rEndX - proEndXRange + offsetX, laneMat.rows),
                ...
                const double& lateralOffset)
{
    if((int)hfLanes.size() == 2) {
        ...
        cv::line(laneMat,
                 cv::Point2d(laneMat.cols * 0.5, laneMat.rows * 0.95),
                 cv::Point2d(laneMat.cols * 0.5, laneMat.rows - 1),
                 cv::Scalar(0, 0, 0),
                 2);
        ...
        cv::putText(laneMat,
                    text_lateralOffset,
                    cv::Point2d((centerPt.x + laneMat.cols * 0.5) / 2, (centerPt.y + laneMat.rows * 0.97) / 2),
