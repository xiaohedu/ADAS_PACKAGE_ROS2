...
void FittingCurve_WLS(const std::vector<cv::Point2d>& pointSet,
                      const int& termNum,
...
void FittingCurve_RANSAC(const std::vector<cv::Point2d>& pointSet,
                         const int& termNum,
                         const int& minDataNum,
                         const int& iterNum,
                         const double& thValue,
                         const int& closeDataNum,
                         cv::Mat& coefs,
                         const cv::Mat& img)
{
    ...
    for(int iter = 0; iter < iterNum; iter++) {
        ...
        //! Different from the linear modeling, the non-linear asks for computation of the distance between point and a
        //polynomial curve.
