                      ...
                      const double& PITCH_ANGLE)
{
    ...
    * The same in the next frame draw the predicted region when laneKalmanIdx exceeds threshold */
    ...
    LaneDetector::DetectLanes(
        grayMat, laneDetectorConf, offsetX, offsetY, hfLanes, postHfLanes, laneKalmanIdx, isChangeLane);
    ...
    if(!hfLanes.empty()) {
        ...
        for(std::vector<LaneDetector::Lane>::const_iterator iter = lanes.begin(); iter != lanes.end(); ++iter) {
            cv::line(laneMat,
                     cv::Point2d(iter->startPoint.x + offsetX, iter->startPoint.y + offsetY),
                     cv::Point2d(iter->endPoint.x + offsetX, iter->endPoint.y + offsetY),
                     CV_RGB(255, 255, 0),
                     3);
        }
    ...
    * @condition 2 : detectLaneFlag == 1 && isChangeLane != -1 && isChangeLane != 1		-> No tracking in the lane
    *departure out of limitation
    * @condition 3 : detectLaneFlag == 2 && changeDone == 1 	-> No tracking in first lane change without init */
    std::vector<cv::Vec2f> preHfLanes;
    postHfLanes.clear();

    if((detectLaneFlag == 0 && initDone == 0) || (detectLaneFlag == 1 && isChangeLane != -1 && isChangeLane != 1) ||
       (detectLaneFlag == 2 && changeDone == 1)) {
        LaneDetector::TrackLanes_KF(grayMat,
                                    laneKalmanFilter,
    ...
    * Lane change happened, lateralOffset is set to lastlateralOffset */
    ...
    if(detectLaneFlag == 0 && initDone == 0) {
        ...
        if(!postHfLanes.empty()) {
            // std::cout << "postHfLanes >> theta_L: " << postHfLanes[0][1] << " theta_R: " << postHfLanes[1][1] <<
            // std::endl;
    ...
    if(!postHfLanes.empty()) {
        ...
        if(heightVP > bottom || distCornerPt < minDIST || distCornerPt > maxDIST) {
            LaneDetector::InitLaneKalmanFilter(laneKalmanFilter, laneKalmanMeasureMat, laneKalmanIdx);
            postHfLanes.clear();
            isTrackFailed = 1;
        }
    ...
    if(!isTrackFailed) {
        if(!postHfLanes.empty() && laneKalmanIdx >= TH_KALMANFILTER) {
            LaneDetector::GetMarkerPoints(
                laneMat, postHfLanes, vanishPt, leftCornerPt, rightCornerPt, offsetX, offsetY);
            cv::circle(laneMat, vanishPt, 4, CV_RGB(255, 0, 255), 2);

            LaneDetector::HfLanetoLane(laneMat, postHfLanes, postLanes);
            for(std::vector<LaneDetector::Lane>::const_iterator iter = postLanes.begin(); iter != postLanes.end();
                ++iter) {
                line(laneMat,
                     cv::Point2d(iter->startPoint.x + offsetX, iter->startPoint.y + offsetY),
                     cv::Point2d(iter->endPoint.x + offsetX, iter->endPoint.y + offsetY),
            ...
            if(hfLanes.size() != 2 || std::abs(lateralOffset - lastLateralOffset) > 0.1) {
                ...
                cv::putText(
                    laneMat, text, cv::Point(laneMat.cols / 2, 20), cv::FONT_HERSHEY_SIMPLEX, 0.7, CV_RGB(0, 255, 0));
            } else {
                LaneDetector::DrawMarker(laneMat, offsetX, offsetY, hfLanes, lateralOffset);
                sprintf(text, "Detection Data");
                cv::putText(
                    laneMat, text, cv::Point(laneMat.cols / 2, 20), cv::FONT_HERSHEY_SIMPLEX, 0.7, CV_RGB(255, 255, 0));
            }
        }
