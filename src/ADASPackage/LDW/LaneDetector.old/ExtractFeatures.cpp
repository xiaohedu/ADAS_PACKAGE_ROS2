...
void ExtractFeatures(const cv::Mat& mat,
                     const std::string& detectorMethod,
                     std::vector<cv::KeyPoint>& keypoints,
                     const std::string& extractorMethod,
                     cv::Mat& descriptor,
                     cv::Mat& outMat)
{
    ...
    if(extractorMethod == "SIFT" || extractorMethod == "SURF" || extractorMethod == "ORB" ||
       extractorMethod == "BRISK" || extractorMethod == "BRIEF" || extractorMethod == "Opponent") {
...
// Matching features in one image with others
...
void ExtractHOG(const cv::Mat& mat)
{
    ...
    cv::HOGDescriptor hog = cv::HOGDescriptor(cv::Size(resizedMat.cols, resizedMat.rows),
                                              cv::Size(pixelsCell * 2, pixelsCell * 2),
                                              cv::Size(pixelsCell, pixelsCell),
                                              cv::Size(pixelsCell, pixelsCell),
                                              9,
                                              0,
                                              -1,
...
void GetVisualHOG(const cv::Mat& mat, const int& pixels, std::vector<float>& descriptors, cv::Mat& visualHOG)
{
    ...
    for(int celly = 0; celly < cellsY; celly++) {
        for(int cellx = 0; cellx < cellsX; cellx++) {
            ...
            cv::rectangle(visualHOG,
                          cv::Point(drawX * zoom, drawY * zoom),
                          cv::Point((drawX + cellSize) * zoom, (drawY + cellSize)) * zoom,
                          CV_RGB(100, 100, 100),
                          1);
...
void HOG_trans(const cv::Mat& imageIPM)
{
    ...
    cv::HOGDescriptor hogtest =
        cv::HOGDescriptor(cv::Size(1, 1), cv::Size(1, 1), cv::Size(1, 1), cv::Size(1, 1), binSize);
