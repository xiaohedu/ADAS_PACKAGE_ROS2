...

{
 ...
 /* cout << "Win width: " << args.win_width << endl;
  ...
  /* the actual run function is now inside the subscription itself. from the begining of the subscription  
    we recieve the current frame and work with it based on the number of provided classifiers in the classifiers.txt file
    then publish at the end the img_out which is the output image after all the classifiers are processed for the current frame 
...
//     cpu_hog.setSVMDetector(detector);
	...
	while (getline(file_classifiers_in, classifier_line)) {
		...
		classifier_line = classifier_line.substr(0, classifier_line.find("#")); // anything written after symbol # will be treated as a comment till end of line
		colors = classifier_line.substr(classifier_line.find(";") + 1 , classifier_line.find("$") );// the tag that will be used to generate the .txt file for the negative data mining will come after the dollar sign
