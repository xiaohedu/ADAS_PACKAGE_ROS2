...
public:
	...
	ADAS_CommandServer (const std::shared_ptr<Message>&  msg_android, rclcpp::executors::SingleThreadedExecutor &exec, const std::string & node_name = "Command_node")
    : Node(node_name, true), msg_android_(msg_android)

	{
		...
		src_pub_ = this->create_publisher<adas_interfaces::msg::SourceParameters>("ADAS_command_pipeline", custom_qos_profile);
...
/*SEND BACK THE PIPELINECONFIG*/
		      if (msg_android_->messagetype() == MessageType::PipeLine_Config)
		   {
		    	  ...
		    	  adas_interfaces::msg::SourceParameters::UniquePtr
		    	  msg (new adas_interfaces::msg::SourceParameters());
	...
	if (Pipeline_spin_request)
	{
	    ...
	    ///////////////////////////////////////////////////////////////////////////
			...
			//Source_ADAS::path_=config.source_folder();
			 ...
			 auto streamer_node_with_ldw   =   std::make_shared<Streamer>("lanedetect_image", android_msg->pipeline_config().ip(), 5000);
